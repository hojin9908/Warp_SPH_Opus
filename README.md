...
source/read_input.py        SOPHIA 형식 입자 입력 파일 읽기/쓰기
...
```bash
python main.py --mode forward                                  # 설정값으로 생성
python main.py --mode forward --input_file input/input.txt     # 파일에서 읽기
```

파일 형식은 **SOPHIA 의 `input/input.txt` 와 같다.**

```
1	2	4	5	7	9	8
0.000000e+00	0.000000e+00	0.000000e+00	0.000000e+00	4.000000e-01	2.600000e-02	1
0.000000e+00	2.000000e-02	0.000000e+00	0.000000e+00	4.000000e-01	2.600000e-02	1
...
```

- **1행은 컬럼 ID 헤더**다. 그 자리에 어떤 물리량이 들어 있는지를 숫자로 말한다.
- 2행부터 입자 하나씩, 헤더와 같은 순서로 값을 탭으로 나열한다.
- 컬럼 순서가 자유롭고, 필요한 것만 넣어도 된다.
...
| 7 | m | 읽어서 설정을 덮어쓴다. 입자마다 다르면 거부한다 |
| 8 | p_type | **0 = 경계, 1 = 유체** (SOPHIA 규약) |
| 9 | h | 읽어서 설정을 덮어쓴다. 입자마다 다르면 거부한다 |
| 그 외 | — | 무시하고 한 번 알려 준다 |

질량과 h 는 이 solver 가 전역 스칼라로 쓴다. 파일이 입자마다 다른 값을 주면
조용히 무시하지 않고 **거부한다.** 아직 지원하지 않는 기능이라는 것을 알리는 편이
틀린 물리를 돌리는 것보다 낫다.

`--dump_input <경로>` 로 생성한 입자를 이 형식으로 저장할 수 있다. 저장했다가
손으로 고쳐 다시 읽는 식으로 쓴다. 저장소의 `input/input.txt` 가 기본 설정으로
뽑아 둔 예제다.

```bash
python main.py --mode forward --dump_input input/my_case.txt   # 뽑아서
# ... 편집기로 고친 뒤 ...
python main.py --mode forward --input_file input/my_case.txt   # 다시 읽는다
```

라운드트립은 비트 단위로 같다 (생성 -> 저장 -> 읽기 -> 시뮬레이션 결과
`max|diff| = 0`). 유체 입자에 `ux = 2.0` 을 손으로 넣어 보면 400 스텝 뒤 선단이
`x = 0.796` 에서 `1.173` 으로 옮겨간다 — 파일이 실제로 물리를 몬다.
...
이웃 탐색은 미분하지 않는다 (7절). 그래도 그래디언트가 맞는 이유는 **이웃이
...
| `--input_file` | (없음) | SOPHIA 형식 입자 입력 파일. 비우면 설정값으로 생성 |
| `--dump_input` | (없음) | 생성한 입자를 SOPHIA 형식으로 저장할 경로 |
